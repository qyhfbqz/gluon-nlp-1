...
class MySoftmaxCrossEntropy(gluon.loss.Loss):
    ...
    def hybrid_forward(self, F, predict_begin, predict_end, label_begin, label_end):
...
class QANet(gluon.HybridBlock):
    ...
    def __init__(self, **kwargs):
        ...
        self.char_conv = gluon.nn.HybridSequential()
        with self.char_conv.name_scope():
            self.char_conv.add(
                gluon.nn.HybridLambda(
                    lambda F, x: F.transpose(x, axes=(1, 0, 2))
                )
            )
            self.char_conv.add(
        ...
        with self.char_emb.name_scope():
            self.char_emb.add(
                gluon.nn.HybridLambda(lambda F, x: F.flatten(x))
            )
            self.char_emb.add(
                gluon.nn.Embedding(
        ...
        self.project = gluon.nn.HybridSequential()
        with self.project.name_scope():
            ...
            self.project.add(gluon.nn.Dropout(opt.layers_dropout))
    ...
    def hybrid_forward(self, F, context, query, context_char, query_char):
        ...
        query_mask = query > 0
        context_max_len = int(F.sum(context_mask, axis=1).max().asscalar())
        query_max_len = int(F.sum(query_mask, axis=1).max().asscalar())
        ...
        # char embedding
        context_char_emb = self.char_emb(context_char)
        query_char_emb = self.char_emb(query_char)
        ...
        # predict layer
        begin_hat = F.flatten(self.predict_begin(F.concat(M_0, M_1, dim=-1)))
        end_hat = F.flatten(self.predict_end(F.concat(M_0, M_2, dim=-1)))
...
class CoAttention(gluon.HybridBlock):
    ...
    def __init__(self, **kwargs):
        ...
        with self.name_scope():
            self.weight_for_context = gluon.nn.Dense(
                units=1,
                flatten=False,
                weight_initializer=Xavier(),
                use_bias=False
            )
            self.weight_for_query = gluon.nn.Dense(
                units=1,
                flatten=False,
                weight_initializer=Xavier(),
                use_bias=False
            )
            self.weight_for_multiply = self.params.get(
    ...
    def hybrid_forward(self, F, context, query, context_mask, query_mask,
                       context_max_len, query_max_len, weight_for_multiply, bias):
        ...
        similarity = self._calculate_trilinear_similarity(
            F, context, query, context_max_len, query_max_len, weight_for_multiply, bias)
    ...
    def _calculate_trilinear_similarity(self, F, context, query, context_max_len, query_max_len,
        ...
        subres0 = F.tile(self.weight_for_context(
            context), [1, 1, query_max_len])
        subres1 = F.tile(F.transpose(
            self.weight_for_query(query), axes=(0, 2, 1)), [1, context_max_len, 1])
        subres2 = F.batch_dot(weight_for_multiply * context,
